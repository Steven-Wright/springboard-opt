...
WITH add_dept_teach AS (
	SELECT REGEXP_SUBSTR(crsCode, '\[a-zA-Z]+') AS dept, crsCode, semester FROM Teaching AS t
),
fix_teach AS (
	SELECT ROW_NUMBER() OVER(PARTITION BY dept) AS m, t.* FROM add_dept_teach AS t
),
add_cols_trans AS (
	SELECT REGEXP_SUBSTR(crsCode, '\[a-zA-Z]+') AS dept, RAND() as rand, crsCode, semester FROM Transcript AS t
),
fix_trans AS (
	SELECT DENSE_RANK() OVER(PARTITION BY dept ORDER BY rand) AS m, dept, crsCode, semester FROM add_cols_trans AS t
)

SELECT
	fix_teach.crsCode AS teach_crsCode,
	fix_teach.semester AS teach_semester,
	fix_trans.crsCode AS trans_crsCode,
	fix_trans.semester AS trans_semester
FROM fix_teach
JOIN fix_trans USING (dept, m));

UPDATE Teaching JOIN table2 ON teach_crsCode = Teaching.crsCode AND teach_semester = Teaching.semester
SET Teaching.crsCode = trans_crsCode, Teaching.semester = trans_semester;
*/
